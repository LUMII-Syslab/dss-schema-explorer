...
-- @param command komandai atbilstošais objekts
...
-- @return iezīmēto objektu kolekcija
...
function activate_element(element)	
	...
	execute_cmd("ActiveElementCmd", {element = element, graphDiagram = diagram})
end
...
---Navigē no elementa uz diagrammu
-- @param elem elements, no kura navigēs
...
---Ja ir iespējams, navigē no elementa uz diagrammu, ja ne, tad atver dialogu logu
-- @param elem elements, no kura navigēs, vai, kuram atvērs dialogu logu
...
---Atver diagrammu
-- @param diagram diagrammu, kuru atvērs
...
function call_elemType_proc_with_supertypes(elemType, attr_name, ...)
	local func_names = get_translets(elemType, attr_name)
...
function execute_translets(obj, attr_name, ...)
	local func_names = get_translets(obj, attr_name)
	if func_names ~= "" and #func_names > 0 then
		return execute_translet(func_names, ...)
	end
end
...
function get_translets(elem_type, attr_name)
	local proc_names = {}
		...
		if elem_type:size() > 0 then
			...
			if #proc_names == 0 then
				...
				if super_type:size() > 0 then
					proc_names = get_translets(super_type, attr_name)
				end
			end
		end
	end
...
function getfield (f)
  ...
  for path_step in string.gfind(f, pattern) do
    ...
    if v and v[path_step] then
      ...
      if is_file_in_lua_path(lua_package_search_path, path) then
        v = require(path)
        if v == true then
...
-- @param function_path ceļš uz izpildāmo funkciju
-- @param ... paremetri, kas tiks nodoti izpildāmajai funkcijai
...
-- @param source_elem elements, no kura kopē
-- @param target_elem elements, kurā kopē
...
-- @param obj objekts, kuru klonē 
-- @return objekts, kas ir padotā objekta klons
...
---Sameklē elementa objektu, kuram šis compartments pieder
...
-- @param obj objekts, kura klases nosaukumu meklē
...
-- @param obj objekts, kura kodu ģenerē
...
---Maina konkrētas elementa stila vērtības
-- @param elem elements, kuram maina stilu
-- @param list saraksts ar atribūtiem, kurus maina
...
-- @param add_new norāda, vai veidot jaunu Tag objektu, vai arī pārrakstīt kādu no jau esošajiem ar šādu identifikatoru
-- @return Tag objekts
...
---Sameklē objekta tagus pēc identifikatora
-- @param obj objekts, kuram meklē tagu
-- @param key Tag identifikators
-- @return Tag objekts
...
---Pārzīmē līnijas
-- @param elems elementu kopa, kuriem ir jāpārzīmē līnijas
...
---Atver stila kasti un, ja vajag nomaina, aktīvā elementa stilu
...
-- izdrukā compartment koku
-- tipiskais lietojums iekš main ierakstam log_compart_tree(utilities.active_elements())
-- izdrukās aktīvā elementa compartment apakškoku nospiežot L
...
---Uzstāda diagrammas nosaukumu
-- @param diagram diagramma, kurai nomaina nosaukumu
-- @param name nosaukums
-- @param is_refresh_needed norāda, vai atbilstošā koka virsotne ir jāpārzīmē
function set_diagram_caption(diagram, name, is_refresh_needed)
	...
	if get_tags(diagram_type, "IsTreeNode"):attr("value") == "true" then
		...
		if node:is_empty() then
			--local parent = t.get_selected_tree_node()
			local parent = utilities.current_diagram():find("/nodeParent")
...
---Sameklē diagrammas objektu, kurai šis elements pieder
-- @param elem Element objekts, no kura meklē diagrammu
-- @return diagrammas objekts
...
-- @param compart Compartment objekts, no kura meklē diagrammu
-- @return diagrammas objekts
...
---Izlīdzina aktīvās diagrammas kastes
...
-- @param list saraksts
-- @return saraksts pretējā secībā
...
function garbage_collection()
	local toolbars = lQuery("Toolbar:not(:has(/graphDiagram))")
		toolbars:find("/toolbarElement"):delete()
	toolbars:delete()

	local popUpDiagrams = lQuery("PopUpDiagram")
		popUpDiagrams:find("/popUpElement"):delete()
	popUpDiagrams:delete()
