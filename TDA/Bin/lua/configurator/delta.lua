...
function dump_configurator()
	...
	table.insert(list_of_code, generate_diagram_type_code(lQuery("GraphDiagram/target_type"):log("id")))
...
function process_target_elem_type(elem)
	...
	local palette_type_id =  diagram_type:find("/paletteType"):id() --*** 2000103355
			...
			for index, id in pairs(id_table) do
				if index ~= "filter" then
					if count == 0 then
						...
						res = res .. ':find("/' .. index .. '[id = ' .. id .. ']"):log("id")'
					end
