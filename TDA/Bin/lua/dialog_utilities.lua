...
function set_field_ok(field, is_refresh_needed)
...
function set_error_field(field, hint_text, is_refresh_needed)
	field:attr({outlineColor = "255", hint = hint_text})
	if is_refresh_needed then
		execute_d_command(field, "Refresh")
	end
end
...
function check_field_value_int_and_chars(table)
	...
	if type(nr_value) == "number" then
		...
		if buls == "false" then
			set_error_field(field, "Error: Value is not an integer!")
		end
	end
	execute_d_command(field, "Refresh")
end
	...
	if container ~= nil then 
		container:find("/component"):each(function(component)
			if component:is_not_empty() then
				delete_container_components(component)
				component:delete()
			end
		end)
	end
...
function close_form(form_id)
	...
	local form
	if form_id == nil then
		form = ev:find("/source/defaultButtonForm")
	else
