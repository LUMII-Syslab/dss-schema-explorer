...
function KeyDown()
	...
	if key_combination == "`" then
		display_console()
	elseif key_combination == "A" then
		utilities.execute_fn("configurator.configurator.automatic_migration")
	...
	elseif key_combination == "C" then
		if not config_properties.get_config_value("is_configurator_hidden") then
			...
			utilities.execute_fn("interpreter.RClick.show_specificationDgr", lQuery("GraphDiagramType[id = 'specificationDgr']/graphDiagram:first()"))
		end
	else
