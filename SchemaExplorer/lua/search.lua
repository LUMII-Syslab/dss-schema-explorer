...
function search()
	...
	local hbox = d.add_component(form, {id = "find_hbox"}, "D#HorizontalBox")
		local vbox_for_params = d.add_component(hbox, {id = "button_vbox", verticalAlignment = -1, horizontalAlignment = -1}, "D#VerticalBox")
			...
			local hbox_checkbox_row = d.add_component(vbox_for_params, {id = "checkbox_hbox", verticalAlignment = -1, horizontalAlignment = -1}, "D#HorizontalBox")
				local vbox_for_checkboxes_1 = d.add_component(hbox_checkbox_row, {id = "button_vbox1", horizontalAlignment = -1}, "D#VerticalBox")
					d.add_component_with_handler(vbox_for_checkboxes_1, {id = "case_sensitive"}, "D#CheckBox", {Change = "lua.search.remove_tag"})	
					d.add_component_with_handler(vbox_for_checkboxes_1, {id = "whole_world_only"}, "D#CheckBox", {Change = "lua.search.remove_tag"})				
					d.add_component_with_handler(vbox_for_checkboxes_1, {id = "class_names_only", checked = true}, "D#CheckBox", {Change = "lua.search.remove_tag"})				
				...
				local vbox_for_checkboxes_3 = d.add_component(hbox_checkbox_row, {id = "button_vbox3", horizontalAlignment = -1}, "D#VerticalBox")
					d.add_component_with_handler(vbox_for_checkboxes_3, {id = "pattern"}, "D#CheckBox", {Change = "lua.search.pattern_click"})
					d.add_component_with_handler(vbox_for_checkboxes_3, {id = "active_diagram", checked = true}, "D#CheckBox", {Change = "lua.search.remove_tag"})	
		...
		local vbox_for_buttons = d.add_component(hbox, {id = "button_vbox"}, "D#VerticalBox")
			local find_button = d.add_component_with_handler(vbox_for_buttons, {id = "find_button", caption = "Find"}, "D#Button", {Click = "lua.search.find"})
			local prev_button = d.add_component_with_handler(vbox_for_buttons, {id = "prev_button", caption = "Previous"}, "D#Button", {Click = "lua.search.activate_prev_elem"})
			local close_button = d.add_component_with_handler(vbox_for_buttons, {id = "close_button", caption = "Close"}, "D#Button", {Click = "lua.search.close_form"})
	form:link("defaultButton", close_button)
	form:link("eventHandler", utilities.d_handler("Close", "lua_engine", "lua.search.close_form"))
	d.show_form(form)
end
...
function find()
	...
	if tag_:attr("value") ~= search_for then
		...
		if class_names_only == "true" then 
			compartments = lQuery("ElemType[id='SH']/compartType[id='name']/compartment")
		else
