...
function generate_property_diagram_from_prop_dgr(prop_dgr, compart)
	local form = d.add_form({id = prop_dgr:attr("id"),
				caption = (prop_dgr:attr("caption") ~= "" and prop_dgr:attr("caption")) or prop_dgr:attr("id"),
...
function close_form()
	local form = lQuery("D#Event"):find("/source/defaultButtonForm")
...
function fill_performers(list_box)
 list_box:find("/item"):delete()
 lQuery("Izpilditajs"):each(
  function(izp)
   lQuery.create("D#Item",{value=izp:attr("vertiba")}):link("listBox",list_box)
  end
 )
 local perf=lQuery("D#TextArea[maximumWidth=12345]")
 local text=perf:attr("text")
 ...
 for i=1,len,1 do
  ...
  if s~="\n" then
   ...
   if z~="" then
    list_box:link("selected",list_box:find("/item[value="..z.."]"))
    z=""
   end
  end
 end
...
function performersBtnClick()
 -- print "Hi there!"
 --lQuery("GraphDiagram"):link("command", utilities.enqued_cmd("D#Command", {info = "Close"}))
 --local cmd=lQuery.create("ExecTransfCmd",{info="lua.specific.izpilditaji_view"})
 --utilities.enqued_cmd(cmd)
 --izpilditaji_view()
 if not(lQuery.model.class_exists("Izpilditajs")) then
  lQuery.model.add_class("Izpilditajs")
  lQuery.model.add_property("Izpilditajs", "vertiba")
  lQuery.model.add_link("D#Item", "item", "izpilditajs", "Izpilditajs")
 end
 local form = d.add_form({id = "Performers_select", caption = "Izpildītāju atlase", minimumWidth=250})
 local list_box=d.add_component(form, {id = "performers_list", horizontalAlignment = 1}, "D#ListBox")
 list_box:attr("multiSelect",true):attr("id","performers_list_box")
 fill_performers(list_box)
 horizontal_box = d.add_component(form, {id = "button_box", horizontalAlignment = 1}, "D#HorizontalBox")
 button = d.add_component(horizontal_box, {caption = "Aizvērt"}, "D#Button")
 d.add_event_handlers(button, {Click = "lua.interpreter.Properties.close_performers_form"})
 d.show_form(form)
-- list_box:log("multiSelect")
end

function update_performers()
 local perf=lQuery("D#TextArea[maximumWidth=12345]")
 local s=""
 lQuery("D#ListBox[id='performers_list_box']/selected"):each(
  function(item)
   s=s..item:attr("value").."\n"
  end
 )
 if s~="" then s=string.sub(s,1,string.len(s)-1) end
 perf:attr({text=s})
 utilities.refresh_form_component(perf)
 --utilities.active_elements():find()
 perf:find("/compartment"):attr("input",s)
 perf:find("/compartment"):attr("value",s)
 utilities.refresh_only_diagram(utilities.current_diagram())
end

function close_performers_form()
	update_performers()
 	local ev = lQuery("D#Event")
	lQuery("D#Form[id = 'Performers_select']"):link("command", utilities.enqued_cmd("D#Command", {info = "Close"})):link("command", utilities.enqued_cmd("D#Command", {info = "Delete"}))
	ev:delete()
end
...
function make_field(row, prop_row, relative_group)
	...
	elseif row_type == "TextArea+Button" then
		...
		add_called_button(row, {id = "called_button", caption = "<-"}, {Click = "lua.interpreter.Properties.show_called_dgr"})
	elseif row_type == "Performers" then
		local min_height = prop_row:attr("height")
		if min_height == "" or min_height == nil then
			min_height = nil
		end
		input = add_field(row, "D#TextArea", {maximumWidth=12345, minimumWidth = min_width, minimumHeight = min_height, maximumHeight = min_height, readOnly = "true"}, {FocusLost = "lua.interpreter.Properties.update_presentation", MultiLineTextBoxChange = change_proc_name}, relative_group)
		add_called_button(row, {id = global_id, caption = "<-"}, {Click = "lua.interpreter.Properties.performersBtnClick"})
		global_id=global_id+1
		--input:log("id")
		--add_called_button(row, {id = "performers_button", caption = "<-"}, {Click = "lua.specific.izpilditaji_view"})
...
function populate_form_from_compartment(form, compart)
	...
	if row ~= nil and row:is_not_empty() then
		...
		elseif component_type == "Performers" then
			add_textArea_value_from_compartment(component, compart)
...
function update_presentation_from_component(row)
	...
	elseif component_type == "Performers" then
		meta_compart, old_val = update_compartment_value_from_textArea(component)
	...
	if multi_text_box:find("/container"):is_not_empty() then
		...
		multi_text_box:find("/textLine"):each(function(text_line)
			...
			--if is_auto_inserted_text_line(text_line) == "false" then
				...
				if text_line:attr("deleted") == "true" or val == "" then
					--text_line:delete()
					...
					old_val = compart:attr("value")
					update_value_and_set_status_for_multi_field(compart, text_line:attr("text"))
				end
			--end
		end)
...
function build_called_diagram(compart)
	if is_property_diagram_empty(compart) == "false" then
		...
		if proc_decompose ~= nil and proc_decompose ~= "" then
			utilities.execute_translet(proc_decompose, compart)
		end
...
function is_multiple_compartType(compartType)
	...
	if compartType:find("/propertyRow"):attr("rowType") == "TextArea+Button" then
		is_multiple = "true"
--	elseif compartType:find("/propertyRow"):attr("rowType") == "Performers" then
--		is_multiple = "true"
	end
	return is_multiple
end;
